...
### <a name="console-unexpectedly-closes-when-removing-collections"></a>La console se ferme de façon inattendue lors de la suppression de collections

<!-- 4749443 -->
*S’applique à : Configuration Manager, version 1902 avec mise à jour cumulative*

Après avoir connecté le site aux [Analyses du bureau](/sccm/desktop-analytics/connect-configmgr), vous pouvez **Sélectionner des regroupements spécifiques à synchroniser avec les Analyses du bureau**. Si vous supprimez un regroupement et appliquez les modifications, l’ajout immédiat d’un nouveau regroupement provoque une exception non gérée. La console se ferme de façon inattendue.

#### <a name="workaround"></a>Solution de contournement

Lorsque vous supprimez un regroupement, sélectionnez **OK** pour fermer la fenêtre des propriétés. Ouvrez ensuite à nouveau les propriétés pour ajouter un nouveau regroupement sur l’onglet **Connexion d’analyses du bureau**.
...
### <a name="you-cannot-deploy-windows-defenderscep-policies-to-client-devices-without-domain-connectivity"></a>Vous ne pouvez pas déployer des stratégies Windows Defender/SCEP sur les appareils clients sans connectivité de domaine
<!-- 4350561 -->
*S’applique à : Configuration Manager, version 1902 et versions antérieures*

Lorsque des stratégies Windows Defender/SCEP stratégies sont appliquées par le client Configuration Manager, une mise à jour de la stratégie de groupe est requise, ce qui ne fonctionne pas lorsque le domaine est inaccessible. Ce problème a un impact sur les appareils managés via Internet par la passerelle de gestion cloud.
