...
> [!IMPORTANT]  
>  Afin que l’installation Push du client réussisse, vérifiez que tous les prérequis sont respectés. Pour plus d'informations, consultez [Dépendances liées aux méthodes d’installation](/sccm/core/clients/deploy/prerequisites-for-deploying-clients-to-windows-computers#installation-method-dependencies).  
...
3.  Sous l’onglet **Accueil** du ruban, dans le groupe **Paramètres**, choisissez **Paramètres d’installation du client**, puis sélectionnez **Installation Push du client**.  
...
7.  Sous l’onglet **Accueil** du ruban, choisissez l’une des options suivantes :  
    ...
    -   Pour envoyer (push) le client à un ou plusieurs appareils, dans le groupe **Appareil**, choisissez **Installer le client**.  

    -   Pour envoyer (push) le client à un regroupement d’appareils, dans le groupe **Regroupement**, choisissez **Installer le client**.  

8. Dans la page **Avant de commencer** de l’Assistant Installation du client, passez en revue les informations, puis choisissez **Suivant**.  
...
Si Configuration Manager n’est pas installé sur un ordinateur, configurez et attribuez un objet de stratégie de groupe. Cette stratégie de groupe spécifie le nom de serveur du point de mise à jour logicielle.  
...
Si vous n’avez pas étendu le schéma Active Directory, utilisez la stratégie de groupe pour provisionner les paramètres d’installation du client. Ces paramètres s’appliquent automatiquement à toute installation du client basée sur les mises à jour logicielles. Pour plus d’informations, consultez la section [Guide pratique pour provisionner les propriétés d’installation du client](#BKMK_Provision) et l’article [Guide pratique pour attribuer des clients à un site](/sccm/core/clients/deploy/assign-clients-to-a-site).  
...
2.  Développez **Configuration ordinateur**, **Modèles d’administration**, **Composants Windows**, puis choisissez **Windows Update**.  
...
3.  Sous l’onglet **Accueil** du ruban, dans le groupe **Paramètres**, choisissez **Paramètres d’installation du client**, puis **Installation du client en fonction des mises à jour logicielles**.  

4.  Sélectionnez **Activer l’installation du client basée sur les mises à jour logicielles**.  

5.  Si la version du client du site est plus récente que la version du point de mise à jour logicielle, la boîte de dialogue **Version plus récente du package client détectée** s’ouvre. Cliquez sur **Oui** pour publier la version la plus récente.  

    > [!NOTE]  
    >  Si vous n’avez pas déjà publié le logiciel client sur le point de mise à jour logicielle, cette boîte est vide.  
...
##  <a name="BKMK_ClientGP"></a> Installation via la stratégie de groupe 

Utilisez la stratégie de groupe dans Active Directory Domain Services pour publier ou attribuer le client Configuration Manager. Le client s’installe au démarrage de l’ordinateur. Quand vous utilisez la stratégie de groupe, le client s’affiche dans le Panneau de configuration **Ajout/Suppression de programmes** pour permettre à l’utilisateur de procéder à l’installation.  

Utilisez le package Windows Installer **CCMSetup.msi** pour les installations basées sur la stratégie de groupe. Ce fichier se trouve dans le dossier `<ConfigMgr installation directory>\bin\i386` sur le serveur de site. Vous ne pouvez pas ajouter des propriétés à ce fichier pour modifier le comportement à l’installation.  

> [!IMPORTANT]  
>  Vous devez disposer d’autorisations d’**administrateur** pour accéder aux fichiers d’installation du client.  

-   Si vous avez étendu le schéma Active Directory pour Configuration Manager et que vous avez sélectionné l’option **Publier ce site dans Active Directory Domain Services** sous l’onglet **Avancé** de la boîte de dialogue **Propriétés du site**, les ordinateurs clients recherchent automatiquement les propriétés d’installation dans Active Directory Domain Services. Pour plus d’informations, consultez [À propos de la publication des propriétés d’installation du client sur les services de domaine Active Directory](/sccm/core/clients/deploy/about-client-installation-properties-published-to-active-directory-domain-services).  

-   Si vous n’avez pas étendu le schéma Active Directory, pour stocker les propriétés d’installation dans le Registre des ordinateurs Windows, consultez la section [Guide pratique pour provisionner les propriétés d’installation du client](#BKMK_Provision). Le client utilise ces propriétés d’installation lors de l’installation.  

Pour plus d’informations sur l’utilisation une stratégie de groupe dans Active Directory Domain Services pour installer des logiciels, consultez le [Guide pratique pour utiliser la stratégie de groupe pour installer le logiciel à distance](https://support.microsoft.com/help/816102/how-to-use-group-policy-to-remotely-install-software-in-windows-server).  



##  <a name="BKMK_Manual"></a> Installation manuelle

Installez manuellement le logiciel client sur des ordinateurs à l’aide de **CCMSetup.exe**. Recherchez ce programme et ses fichiers de prise en charge dans le dossier **Client** du dossier d’installation de Configuration Manager sur le serveur de site. Le site partage ce dossier sur le réseau de la façon suivante :  

 `\\<Site Server Name>\SMS_<Site Code>\Client\`  

 où `<Site Server Name>` est le nom du serveur de site principal, et `<Site Code>` est le code de site principal auquel le client est attribué. Pour exécuter CCMSetup.exe à partir de la ligne de commande sur le client, connectez-vous à cet emplacement réseau, puis exécutez la commande.  
...
|`SMSSITECODE=AUTO`|Cette propriété de Client.msi spécifie que le client doit essayer de trouver le code de site Configuration Manager à utiliser. Par exemple, en utilisant Active Directory Domain Services.|  
...
Vous avez configuré tous les serveurs de système de site avec un nom de domaine complet d’intranet. Vous avez publié les informations de site sur Active Directory.  

Démarrez par les étapes suivantes sur l’ordinateur client :  
1. Connectez-vous en tant qu’administrateur local  
2. Mappez le lecteur Z: à `\\MPSERVER\SMS_ABC\Client`  
3. Indiquez le lecteur Z: à l’invite de commandes  
...
Configuration Manager prend en charge les scripts d’ouverture de session pour installer le logiciel client Configuration Manager. Utilisez le fichier programme **CCMSetup.exe** dans un script d’ouverture de session pour déclencher l’installation du client.  
...
> [!NOTE]  
>  Vous ne pouvez pas mettre à niveau des clients Configuration Manager 2007 avec cette méthode. Au lieu de cela, utilisez la mise à niveau automatique des clients, qui crée et déploie automatiquement un package contenant la dernière version du client. Pour plus d’informations, consultez [Mettre à niveau les clients](/sccm/core/clients/manage/upgrade/upgrade-clients).  
>   
...
2.  Sous l’onglet **Accueil** du ruban, dans le groupe **Créer**, choisissez **Créer un package à partir d’une définition**.  
...
> [!NOTE]  
> Par défaut, une fois le logiciel client installé, l’appareil se désinscrit d’Intune.
> 
> À compter de la version 1710, les clients ne se désinscrivent pas d’Intune. Ils peuvent disposer à la fois du client Configuration Manager et d’une inscription MDM. Pour plus d’informations, consultez [Vue d’ensemble de la cogestion](/sccm/comanage/overview).  


###  <a name="install-clients-with-intune"></a>Installer des clients avec Intune  

1. Dans Intune, [ajoutez une application métier](https://docs.microsoft.com/intune/lob-apps-windows) contenant le fichier d’installation du client Configuration Manager **ccmsetup.msi**. Ce fichier se trouve dans le dossier suivant sur le serveur de site : `<ConfigMgr installation directory>\bin\i386`  
...
2. Dans l’Éditeur de logiciel Intune, entrez des paramètres de ligne de commande. Par exemple, utilisez la ligne de commande suivante avec un client classique sur l’intranet :  
   ...
   > [!Note]  
   > Pour obtenir un exemple de ligne commande à utiliser avec un client Windows 10 moderne exploitant l’authentification Azure AD, consultez [Préparer des appareils basés sur Internet à la cogestion](/sccm/comanage/how-to-prepare-win10#install-the-configuration-manager-client).  
...
-   Utilisez une méthode d’installation indépendante de Configuration Manager. Par exemple, empaquetez les fichiers sources d’installation du client sur un média amovible et envoyez-les aux utilisateurs pour qu’ils les installent. Les fichiers sources d’installation du client se trouvent dans le dossier `<InstallationPath>\Client` du serveur de site Configuration Manager. Incorporez au média un script à copier manuellement dans le dossier du client. À partir de ce dossier, installez manuellement le client en utilisant CCMSetup.exe et toutes les propriétés de ligne de commande CCMSetup appropriées.  
...
1.  Suivez les instructions de la section [Guide pratique pour installer manuellement des clients Configuration Manager](#BKMK_Manual). Incluez toujours les options suivantes :  
    ...
    -   Paramètre de ligne de commande CCMSetup `/source:<local path to the copied Client folder>`  
    ...
    -   Propriété Client.msi `SMSSIGNCERT=<local path to exported site server signing certificate>`  
...
Utilisez la stratégie de groupe Windows pour provisionner des ordinateurs avec les propriétés d’installation du client Configuration Manager. Ces propriétés sont stockées dans le Registre de l’ordinateur. Le client les lit lors de son installation. Cette procédure n’est généralement pas obligatoire, mais peut s’avérer nécessaire dans certains scénarios d’installation du client, par exemple :  
...
1.  Importez le modèle d’administration **ConfigMgrInstallation.adm** dans un objet de stratégie de groupe nouveau ou existant, à l’aide d’un éditeur comme l’Éditeur d’objets de stratégie de groupe Windows. Recherchez ce fichier dans le dossier `TOOLS\ConfigMgrADMTemplates` du média d’installation de Configuration Manager.  
