...
### <a name="send-a-suggestion"></a>Envoyer une suggestion

Quand vous **envoyez une suggestion**, vous êtes redirigé vers [UserVoice](https://configurationmanager.uservoice.com/), un site web tiers servant à partager vos idées. L’équipe produit de Configuration Manager utilise les valeurs d’état UserVoice suivantes :

- **Noté** : nous comprenons la demande et elle est judicieuse. Nous l’avons ajoutée à notre backlog.
- **Planifié** : nous avons commencé rédiger du code concernant cette fonctionnalité et nous espérons qu’elle figurera dans une build de préversion technique dans les prochains mois.
- **Démarré** : la fonctionnalité est désormais en préversion technique. Vérifiez-le et faites-nous part de vos commentaires. Faites-nous savoir si la fonctionnalité est, ou non, sur la bonne voie. Placez des commentaires supplémentaires dans la section des commentaires de la demande d’origine pour que d’autres personnes les voient et fassent part de leurs observations. Nous les lirons et utiliserons les commentaires pour essayer d’améliorer la fonctionnalité.
- **Terminé** : la première version de la fonctionnalité se trouve dans une build de production. Cet état ne signifie pas que nous avons totalement terminé le déploiement de la fonctionnalité et que nous ne l’améliorerons plus. Cela signifie que la version v1 de la fonctionnalité se trouve dans une build de production, et que vous pouvez commencer à réellement l’utiliser. Nous la marquons comme terminée car :
  - Nous voulons que vous sachiez que la fonctionnalité est prête pour la production.
  - Nous voulons vous restituer vos votes UserVoice pour vous permettre de les utiliser sur d’autres éléments.
  - Vous pouvez déposer de nouvelles demandes de changement de conception pour cette fonctionnalité afin de nous aider à savoir quelle est la prochaine amélioration la plus importante que nous devons lui apporter.

### <a name="information-sent-with-feedback"></a>Informations envoyées avec les commentaires

Quand vous **envoyez un sourire** ou que vous **envoyez un smiley mécontent**, les informations suivantes sont envoyées avec les commentaires :
...
5. Exécutez la commande suivante : `UploadOfflineFeedback.exe -f c:\folder\location_of.zip`
    ...
    - Si vous le souhaitez, vous pouvez ajouter les paramètres suivants :
...
## <a name="bkmk_feedbackid"></a> Confirmation de commentaires de la console

<!--3556010-->
À compter de la version 1902, quand vous envoyez des commentaires par le biais de la console Configuration Manager ou d’UploadOfflineFeedback.exe, un message de confirmation s’affiche. Ce message inclut un **ID de commentaire** que vous pouvez donner à Microsoft comme identificateur de suivi.

- Pour copier l’**ID de commentaire**, sélectionnez l’icône de copie en regard de l’ID ou utilisez le raccourci clavier **CTRL** + **C**.
  - Cet ID n’est pas stocké sur votre ordinateur : veillez donc à le copier avant de fermer la fenêtre.
- Le fait de cliquer sur **Ne plus afficher ce message** supprime la boîte de dialogue et l’empêche de réapparaître.

   ![Accusé de réception des commentaires émis par la console dans Configuration Manager 1902](media/1902-feedback-id-example.png)
- L’outil de commande **UploadOfflineFeedback** écrit la valeur **FeedbackID** dans la console, sauf si -s ou --silent est utilisé.

  ![Accusé de réception des commentaires émis par UploadOfflineFeedback.exe dans Configuration Manager 1902](media/1902-offline-feedback-id-example.png)
...
Continuez à utiliser la [page UserVoice](https://configurationmanager.uservoice.com/) afin de voter pour de nouvelles idées de fonctionnalités dans Configuration Manager. L’équipe produit de Configuration Manager utilise les valeurs d’état UserVoice suivantes :
