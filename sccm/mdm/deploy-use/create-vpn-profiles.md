...
<sup>1</sup> depuis la version 1802, l’utilisation du type de connexion Cisco AnyConnect varie.<!--1357393-->  
...
6. Dans la page **Méthode d’authentification**, spécifiez les paramètres suivants :  
   ...
   - **Méthode d’authentification**: Sélectionnez la méthode d’authentification qui utilise la connexion VPN. Les méthodes disponibles peuvent varier selon le type de connexion, comme indiqué dans le tableau suivant.  
     ...
     - **Sélectionner un certificat client pour l'authentification du client** : Sélectionnez le client créé précédemment [certificat SCEP](create-pfx-certificate-profiles.md) qui est utilisé pour authentifier la connexion VPN.   
